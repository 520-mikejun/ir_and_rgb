...
namespace mvs_camera
{
  ...
  void histogramStretching(cv::Mat &image)
  {
    // 转换到 YUV 色彩空间
    cv::Mat img_yuv;
    cv::cvtColor(image, img_yuv, cv::COLOR_BGR2YUV);
    ...
    // 转换回 BGR 色彩空间
    cv::cvtColor(img_yuv, image, cv::COLOR_YUV2BGR);
  }
  ...
  void MvsCamera::StartGrabbing()
  {
    ...
    while (ros::ok())
    {
      ...
      // Step 1: 读取 mono12 格式的图像数据并创建 OpenCV Mat
      cv::Mat image(cv::Size(stOutFrame.stFrameInfo.nWidth, stOutFrame.stFrameInfo.nHeight), CV_16UC1, stOutFrame.pBufAddr);

      // 将 12 位图像数据转换为 8 位图像
      cv::Mat image_8bit;
      image.convertTo(image_8bit, CV_8UC1, 255.0 / 4095.0);

      // 转换为 RGB 格式
      cv::Mat image_rgb;
      cv::cvtColor(image_8bit, image_rgb, cv::COLOR_GRAY2BGR);
      ...
      cv::normalize(image_rgb, image_normalized, 0, 250, cv::NORM_MINMAX);

      // 使用伪彩色调色板转换图像
      cv::Mat image_colormap;
      cv::applyColorMap(image_normalized, image_colormap, cv::COLORMAP_JET);
      ...
      histogramStretching(rectify_image);
      // 将矫正后的图像转换为 ROS 图像消息
