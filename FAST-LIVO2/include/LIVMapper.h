...
public:
  ...
  //定义红外的成员变量
  bool ir_img_en;                      // 是否启用红外图像处理的标志
  bool ir_only_mode_en;                // 是否启用仅红外-激光雷达模式
  bool ir_vis_fusion_en;               // 是否启用红外-可见光融合
  string ir_img_topic;                 // 红外图像话题名称
  double ir_img_time_offset;           // 红外图像时间偏移

  // 红外相机数据缓冲区
  cv::Vec3b ir_image_color[IR_HMAX][IR_WMAX];  // 全局红外图像数据数组  // 全局红外图像数据数组
  double last_timestamp_ir_img = 0.0;     // 最后一帧红外图像时间戳
  std::deque<cv::Mat> ir_img_buffer;      // 红外图像缓冲区
  std::deque<double> ir_img_time_buffer;  // 红外图像时间戳缓冲区
  ros::Publisher pubIRColorCloud;         // 红外彩色点云发布器
  // 红外相机外参
  vector<double> IRextrinT;            // 红外相机到激光雷达的平移向量（向量形式）
  vector<double> IRextrinR;            // 红外相机到激光雷达的旋转矩阵（向量形式）
  V3D IRextT;                          // 红外相机到激光雷达的平移向量（Eigen矩阵形式）
  M3D IRextR;                          // 红外相机到激光雷达的旋转矩阵（Eigen矩阵形式）

  // 红外相机订阅器和发布器
  ros::Subscriber sub_ir_img;          // 红外图像订阅器
  image_transport::Publisher pubIRImage; // 红外图像发布器

  // 红外相机内参和畸变系数
  cv::Mat ir_camera_matrix;            // 红外相机内参矩阵
  cv::Mat ir_dist_coeffs;              // 红外相机畸变系数

  // 添加函数声明
  void ir_img_cbk(const sensor_msgs::ImageConstPtr &msg_in); // 红外图像回调函数
  void processIRImages();              // 处理红外图像
  void handleIRLIO();                  // 处理红外-激光雷达融合建图
  void publish_ir_img(const image_transport::Publisher &pubImage); // 发布红外图像
